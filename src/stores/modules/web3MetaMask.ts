...
const useWeb3Store = defineStore('web3-meta-mask', () => {
  ...
  /**
   * 连接到MetaMask钱包，创建web3实例。
   *
   * @returns {Promise<void>} 一个Promise，解析为用户连接的MetaMask账户数组。
   */
   ...
   *
   * @param {bigint | string} value - 要转换的金额，以 wei 为单位。
   ...
   *
   * @param {bigint | string} value - 要转换的金额，以 ETH 为单位。

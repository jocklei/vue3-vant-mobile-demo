<script setup lang="ts">
  ...
  async function sendSignedTransaction() {
    ...
    const txData = {
      from: from.value,
      to: to.value,
      value: useWeb3.toWei(value.value),
      gasPrice: await useWeb3.getGasPrice(),
      nonce: await useWeb3.getTransactionCount(from.value)
    };

    // 估算gas费用
    txData['gas'] = await useWeb3.estimateGas(txData)

    // 签名
    const tx = await useWeb3.signTransaction(txData, privateKey.value);
    ...
    // 发送交易
    const receipt = await useWeb3.sendSignedTransaction(raw)
...
<template>
  <VanCellGroup :inset="true" class="!mt-16">
    ...
    <van-cell>
      <van-button type="success" size="small" @click="sendSignedTransaction">发起转账</van-button>
    </van-cell>
  </VanCellGroup>
</template>
